...
if filterData == 'Yes':
    ...
    filterOrder = st.number_input("Enter the order of the filter you'd like to use.",
                                  min_value = 1, 
                                  max_value = 7,
                                  value = 4,
                                  step = 1)
    cutoffFreq = st.number_input("Enter the low-pass cutoff frequency you'd like to use for filtering.",
                                 min_value = 1, 
                                 max_value = 50, 
                                 value = 15, 
                                 step = 1)
...
if processButton:
    ...
    match (offset,filterData):
        case ('Yes','Yes'):
            ...
            inputs_dict['sampleFreq'] = sampleFreq
        case ('Yes', 'No'):
            forceData, inputs_dict, offsetValue = offset_correct_force(forceData, inputs_dict, startTime)
        case ('No','Yes'):
            forceData['filtered'] = filter_data(forceData,
